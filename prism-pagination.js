 ...
 *
 * ```
 * <prism-pagination page="[[page]]" count="[[count]]" range="[[range]]" url="[[url]]">
 *   <template slot="first"><a class$="[[css]]" href$="[[url]]">&lt;&lt;</a></template>
 *   <template slot="previous"><a class$="[[css]]" href$="[[url]]">&lt;</a></template>
 *   <template slot="general"><a class$="[[css]]" href$="[[url]]">[[text]]</a></template>
 *   <template slot="current"><a class$="[[css]]">[[text]]</a></template>
 *   <template slot="next"><a class$="[[css]]" href$="[[url]]">&gt;</a></template>
 *   <template slot="last"><a class$="[[css]]" href$="[[url]]">&gt;&gt;</a></template>
 * </prism-pagination>
 * ```
 *
 * @memberof Prhythm
 * @element prism-pagination
 * @customElement
 * @polymer
 * @demo demo/index.html Pagination Demo
 */
class PrismPagination extends PolymerElement {

  static get is() { return 'prism-pagination'; }
  ...
  */
  _computedHidden(page, count, range) {
    ...
    if (!page || isNaN(page * 1) || page * 1 < 1) {
      hidden = true;
      console.warn(`<prism-pagination> property page is invalid: ${page}`);
    }
    if (!count || isNaN(count * 1) || page * 1 < 1) {
      hidden = true;
      console.warn(`<prism-pagination> property count is invalid: ${count}`);
    }
    if (count * 1 === 1) {
      hidden = true;
      console.info(`<prism-pagination> single page detected.`);
    }
  ...
  */
  _render() {
    ...
    if ((templateClass = this._getTemplateType('general'))) {
      let currentTempateClass = this._getTemplateType('current'),
  ...
  */
  _getTemplateType(name) {
    ...
    if (!TemplateClass) {
      ...
      if (!template) {
        console.warn(`[prism-pagination] requires a <template slot="${name}"> child`);
        return null;
      }
...
customElements.define(PrismPagination.is, PrismPagination);
